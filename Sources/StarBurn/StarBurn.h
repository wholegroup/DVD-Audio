...
Revision/Development History:
    ...
    2003-08-01 : * DEVICES_PER_BUS increased from 15 to 63 to support badly written virtual CD/DVD devices.
	...
	2004-03-01 : * StarBurn_CdvdBurnerGrabber_DiscAtOncePQFromFile() updated to support subchannel repair.
	...
	2005-02-19 : * Changed structure UDF_TREE_ITEM. Removed:
	...
	2005-12-02 : * Version change to 5.12.02.
...
//
// Build date packed into 32-bit unsigned long (used as version number)
//

#define BUILD_DATE                                  0x20051202
...
#define NUMBER_OF_SUBCHANNELS                       8       // Subchannels from P to W
...
#define SUBCHANNEL_SIZE_IN_UCHARS                   12      // Subchannel size in UCHARs

#define SUBCHANNEL_PQ_SIZE_IN_UCHARS                16      // PQ subchannel size in UCHARs

#define SUBCHANNEL_RAW_PW_SIZE_IN_UCHARS            96      // Raw P-W subchannel size in UCHARs
...
//
// Other CD/DVD speeds are just multiplier of ( CDVD_SPEED_IN_KBPS_1X * N ) where N is drive speed
//


//
// Top supported CD/DVD speed constant (wildcard)
//
...
//
// 2.4X DVD speed constant (DVD+R and DVD+RW)
//
...
//
// 4X DVD speed constant (DVD-R, DVD-RW, DVD+R, DVD+RW and DVD-RAM)
//
...
//
// 8X DVD speed constant (DVD-R, DVD+R, DVD+RW)
//
...
//
// Top number of tracks on CD/DVD disc
//
...
//
// Structure that represents track set
// <TABLE>
// Member
// ----------                           ---------------
// m__LONG__NumberOfTracks				True number of tracks used
// m__CHAR__TrackName[ ][ ] 			Track names			
// </TABLE>

typedef struct _TRACK_SET
{
	LONG m__LONG__NumberOfTracks;		// True number of tracks used

	CHAR m__CHAR__TrackName[ NUMBER_OF_TRACKS ][ MAX_PATH ]; // Track names

} *PTRACK_SET, TRACK_SET;
...
// m__UCHAR__PSubChannel 				P subchannel bit
// </TABLE>
...
typedef struct _PQ_SUBCHANNEL
{
    ...
    UCHAR m__UCHAR__PSubChannel : 1;	// P subchannel bit
...
// Member                               Definition
// ----------                           ---------------
// EN_SUCCESS                           Nothing really happened, operation completed successfuly
...
// EN_INVALID_RESPONSE                  Something really unsupposed returned
...
// EN_GENERAL_SEEK_ERROR				General seek error on CD/DVD media
// EN_GENERAL_READ_ERROR				General read error on CD/DVD media
...
{
    EN_SUCCESS = 0,                     // Nothing really happened, operation completed successfuly
...
// KIDTYPE_FROMDISK						Node was added from the disc
...
// CN_FILE_TREE_PROGRESS_IGNORE         Item was ingored during processing file tree
...
// CD_CDVD_READ_PROGRESS                CD/DVD read operation completed
// CN_CDVD_WRITE_PROGRESS               CD/DVD write operation completed
// CN_CDVD_BUFFER_STATUS                CD/DVD buffer status information queried
// CN_CDVD_TRACK_BEGIN                  CD/DVD track processing started
// CN_CDVD_TRACK_END                    CD/DVD track processing completed
// CN_CDVD_SPLIT_BEGIN                  CD/DVD split section processing started
// CN_CDVD_SPLIT_END                    CD/DVD split section processing completed
// CN_CDVD_READ_BAD_BLOCK_HIT           CD/DVD read operation had hit a bad (unrecoverable) block
// CN_CDVD_READ_ECCEDC_BAD_BLOCK_HIT    CD/DVD read operation had hit a ECC/EDC bad (recoverable) block
// CN_CDVD_READ_RETRY                   CD/DVD read operation was retried
...
// CN_CDVD_READ_CANCEL_QUERY			CD/DVD media processing plug-in queries cancel status
...
// CN_DVD_TEST_WRITE_DISABLED			Test write is disabled (for DVD+R/RW or multisession DVD-RW)
// CN_CDVD_DPM_BEGIN                  	CD/DVD DPM processing started
// CN_CDVD_DPM_END                    	CD/DVD DPM processing completed
// CD_CDVD_DPM_PROGRESS                	CD/DVD DPM operation progress
// CN_CDVD_VERIFY_PROGRESS              CD/DVD verify operation completed
...
typedef enum _CALLBACK_NUMBER
{
    ...
    CN_FILE_TREE_PROGRESS_IGNORE,       // Item wa ingored during processing file tree
    ...
    CN_CDVD_READ_PROGRESS,              // CD/DVD read operation completed
	...
	CN_CDVD_DPM_PROGRESS,              	// CD/DVD DPM processing progress
...
// READ_MODE_RAW_PQ                     Raw data + PQ subchannel
// READ_MODE_RAW_RAW_PW                 Raw data + raw P-W subchannel
// READ_MODE_PQ                         PQ subchannel only (no main channel data)
// READ_MODE_RAW_PW                     Raw P-W subchannel only (no main channel data)
// </TABLE>
...
typedef struct _NAME_COLLISION_INFO
{
    ...
    NAME_COLLISION_SOLUTION m__NAME_COLLISION_SOLUTION;	// The solution of the collistion (what action has been performed)
...
{
    BOOLEAN m__BOOLEAN__IsValid;        // Is this data valid (structue was really filled)
...
} *PTRACK_INFORMATION, TRACK_INFORMATION;
...
} *PDISC_INFORMATION, DISC_INFORMATION;
...
typedef struct _TOC_INFORMATION 
{
    ...
    FULL_TOC_ENTRY_RAW m__FULL_TOC_ENTRY_RAW__Unsorted[ NUMBER_OF_TRACKS ];	// Full TOC raw entries unsorted
...
typedef struct _DISC_LAYOUT_ENTRY
{
    ...
    BOOLEAN m__BOOLEAN__IsRawRawPW;			// Is this raw + raw P-W subchannel track
...
// m__LONG__NumberOfRawRawPWTracks      Number of raw + raw P-W subchannel tracks
// m__LONG__RawRawPWTrackSize           Raw + raw P-W subchannel track sizes
// </TABLE>
...
typedef struct _DISC_LAYOUT
{
    ...
    LONG m__LONG__NumberOfRawRawPWTracks;						// Number of raw + raw P-W subchannel tracks

    LONG m__LONG__RawRawPWTrackSizeInLBs[ NUMBER_OF_TRACKS ];	// Raw + raw P-W subchannel track sizes			
...
// m__CHAR__TrackName                   Track name (absolute path & name)
...
typedef struct _DAO_DISC_LAYOUT_ENTRY
{
    ...
    CHAR m__CHAR__TrackName[ MAX_PATH ];	// Track name (absolute path & name)
...

{
    ...
    //
    // Dump just generated stirng to the debug output
    //
...
p__PVOID__RegistrationKey - Pointer to registraion key.
...
This function uninitializes burning toolkit. It's expected to be called after very last function call before 
...
recorder directly to the CD/DVD media w/o any other intermediate buffering operations on the hard disk.
...
p__PULONG__SystemError - Pointer to ULONG that will contain the system error (if some will occure).
...
of this operaion (See samples how to use this callback for progress indication and other useful things).
...
Execution status. EN_SUCCESS if the tree was allocated successfuly, E_MEMORY_ALLOCATION_FAILED if there is not enough memory 
to allocate the tree. If the exception number will be E_SYSTEM_CALL_FAILED, variable that SystemError points to will be 
...
stored in the file on the disk or be burn directly to the CD/DVD media w/o any other intermediate operations.
...
Execution status. EN_SUCCESS if the tree was reallocated successfuly, E_MEMORY_ALLOCATION_FAILED if there is not enough memory 
to reallocate the tree. If the exception number will be E_SYSTEM_CALL_FAILED, variable that SystemError points to will be 
filled with system error. If something other then EN_SUCCESS will be returned buffer that ExceptionText will point to will be 
filled with formatted exception message.
...
This function returns ISO9660 or Joliet file tree node power in UCHARs. 
...
Node power in UCHARs. This function cannot fail.


Remarks

Node power in UCHARs can be used to build correct tree to visualise enumeration of all the kids and walk down the tree.
...
Names can be used to build correct tree to visualise enumeration of all the kids and walk down the tree.
...
Names can be used to build correct tree to visualise enumeration of all the kids and walk down the tree. Changing the names
...
This function builds ISO9660 or Joliet file system image from ISO9660 or Joliet file tree (assignes all the logical block 
...
ISO9660 or Joliet image to the file on the disk or to burn this ISO9660 or Joliet image directly to the CD/DVD media.
...
CD/DVD media, 0 for empty disc).

p__LONG__TreeLevelToProcess - Top tree level to process, levels that exceed this value will be ingnored and not included 
...
Execution status. EN_SUCCESS if the operation completed successfuly. If the exception number will be E_SYSTEM_CALL_FAILED, 
...
are passed into the callback function during the operations with file tree object. See ISO9660_XXX constats to verify correct 
string parameter lengthes.
...
can be used instead of StarBurn_ISO9660JolietFileTree_BuildImage. See ISO9660_XXX constats to verify correct string parameter 
lengthes.
...
Used to determine the size of the built file system image so it will be clear will it fit on the CD/DVD media (if burning)
...
// Check for enough space on the media here (either hard disk or CD/DVD)
...
The data that is read can be either stored in the file on the hard disk or directly written to CD/DVD media.
...
p__LONG__IoTransferSizeInUCHARs - Number of UCHARs to read from the image fom the current offset.
...
burn later to CD/DVD media or used with CD/DVD emulation software packages. Also TrackAtOnceFromTree sample can be checked out
to see how ISO9660 or Joliet image can be recorded on the CD/DVD media w/o intermediate buffering on the hard disk.
...
This function seeks the ISO9660 or Joliet file system image from current offset to the very begining so read operation will 
...
either burn later to CD/DVD media or used with CD/DVD emulation software packages. StarBurn_ISO9660JolietFileTree_SeekToBegin 
...
This function creates CD/DVD burner device object. This object will be used later to perform CD/DVD releated actions.
...
p__PCDB_FAILURE_INFORMATION - Pointer to CDB_FAILURE_INFORMATION that will be filled with apropriate values.
...
E_SCSI_CDB_FAILED, CDB_FAILURE_INFORMATION will be filled with apropriate values (CDB that faled, CDB size, SCSI sense data, 
...
image can be burn on the CD/DVD media. 
...
StarBurn_Destroy, PCALLBACK, EXCEPTION_NUMBER, CDB_FAILURE_INFORMATION 
...
This function returns CD/DVD burner device object information. This data can be used to report the capabilities of the device 
to the user.
...
Please see the TrackAtOnceFromTree and TrackAtOnceFromFile samples that will demonstrate how ISO9660 or Joliet file system 
image can be burn on the CD/DVD media and what use can be taked from CdvdBurnerGrabber device information. 


Example

This example allocates CdvdBurnerGrabber object, retreives device information and destroys the device object after it's not 
needed any more.
...
This function returns CD/DVD burner device object extended information. This data can be used to report the capabilities of the
device to the user.
...
Please see the TrackAtOnceFromTree and TrackAtOnceFromFile samples that will demonstrate how ISO9660 or Joliet file system 
image can be burn on the CD/DVD media and what use can be taked from CdvdBurnerGrabber device extended information. 


Example

This example allocates CdvdBurnerGrabber object, retreives device extended information and destroys the device object after 
...
Please see the TrackAtOnceFromTree and TrackAtOnceFromFile samples that will demonstrate how ISO9660 or Joliet file system 
image can be burn on the DVD+R(W) media and what use can be taked from CdvdBurnerGrabber device extended information. 
...
This example allocates CdvdBurnerGrabber object, retreives device extended extended information and destroys the device object 
...
This function tests is CD/DVD burner device object unit ready or not. This information can be used later to start I/O 
operations.
...
Please see the TrackAtOnceFromTree and TrackAtOnceFromFile samples that will demonstrate how ISO9660 or Joliet file system 
image can be burn  on the CD/DVD media and what StarBurn_CdvdBurnerGrabber_TestUnitReady can be used for. 
...
This function tests is CD/DVD burner device object unit ready or not. This information can be used later to start 
...
p__ULONG__NumberOfSecondsToWait - Number of seconds to wait until the unit would not become into ready state.
...
system image can be burn on the CD/DVD media and what StarBurn_CdvdBurnerGrabber_TestUnitReady and it's more extended
"clone" StarBurn_CdvdBurnerGrabber_TestUnitReadyEx can be used for. 
...
This function tests is CD/DVD burner device supports BUP (Buffer Underrun Protection) or not and what is BUP current status 
...
Please see the TrackAtOnceFromTree and TrackAtOnceFromFile sample that will demonstrate how ISO9660 or Joliet file system image
 can be burn on the CD/DVD media and what StarBurn_CdvdBurnerGrabber_GetBUP can be used for. 
...
This function sets BUP (Buffer Underrun Protection) on CD/DVD burner device that supports BUP. BUP must be enabled for 
...
p__BOOLEAN__IsBUPEnabled - BOOLEAN variable that keeps TRUE if BUP is must be enabled on the CD/DVD device, FALSE if BUP 
must be disabled.
...
Please see the TrackAtOnceFromTree and TrackAtOnceFromFile samples that will demonstrate how ISO9660 or Joliet file system 
image can be burn on the CD/DVD media and what StarBurn_CdvdBurnerGrabber_GetBUP can be used for. 
...
This function sets current read and write speeds on CD/DVD burner device This speeds later will be used during all read and 
write operations.
...
StarBurn_CdvdBurnerGrabber_SetSpeeds can be used for setting CD/DVD device read and write speeds. 
...
// Try to set top supported CD/DVD read/write speeds on the device
l__EXCEPTION_NUMBER =
...
This function gets current read and write and top supported read and write speeds on CD/DVD burner device. This speeds 
...
can be used to determine current and top supported CD/DVD device read and write speeds and how 
...
// Try to get current and top supported CD/DVD read/write speeds on the device
l__EXCEPTION_NUMBER =
...
This function sends OPC (Optimum Power Calibrarion) for current write speed on CD/DVD burner device and current CD/DVD media. 
...
Please see the TrackAtOnceFromTree and TrackAtOnceFromFile samples that will demonstrate how ISO9660 or Joliet file system 
image can be burn on the CD/DVD media and how StarBurn_CdvdBurnerGrabber_SendOPC can be used to set optimum laser level on CD/DVD
device for current write speed and CD/DVD media. 


Example

This example allocates CdvdBurnerGrabber object, sends optimum power calibration for current write speed and current CD/DVD 
...
StarBurn_ISO9660JolietFileTree_Create or ISO image with current write speed on CD/DVD burner device in Session-At-Once mode.
...
on the CD/DVD media and with the help of StarBurn_CdvdBurnerGrabber_SessionAtOnce. SessionAtOnceFromFile sample shows how to
burn ISO image. 


Example

This example allocates CdvdBurnerGrabber object, records the ISO9660 or Joliet file system image from file tree to the CD/DVD 
media and destroys the device object after it's not needed any more.
...
This function records raw image with current write speed on CD/DVD burner device in raw Session-At-Once mode.
...
Please see the SessionAtOnceRawRawPW sample that will demonstrate how how to raw image. 
...
This function records ISO9660 or Joliet file system image located in a virtual file tree created with 
StarBurn_ISO9660JolietFileTree_Create with current write speed on CD/DVD burner device in Track-At-Once mode.
...
on the CD/DVD media and with the help of StarBurn_CdvdBurnerGrabber_TrackAtOnceFromTree.
...
audio CD is mastered) with current write speed on CD/DVD burner device in Track-At-Once mode.
...
CD/DVD media and with the help of StarBurn_CdvdBurnerGrabber_TrackAtOnceFromFile with currently set write speed and currently set 
optimum power calibration. 


Example

This example allocates CdvdBurnerGrabber object, records the ISO9660 or Joliet file system image to the CD/DVD media and 
...
full set of sound files if an audio CD is mastered) with current write speed on CD/DVD burner device in Disc-At-Once PQ mode.
...
p__BOOLEAN__IsSubChannelRepairRequired -- BOOLEAN set to TRUE if you want library to repair broken subchannel in the image
...
file or full set of sound files if an audio CD is mastered) can be burn on the CD/DVD media and with the help of 
...
This function records ISO9660 or Joliet file system image located in a file on the hard disk (also it can be one sound file or 
full set of sound files if an audio CD is mastered) with current write speed on CD/DVD burner device in Disc-At-Once raw P-W mode.
...
This function records ISO9660 or Joliet file system image located in a pipe with current write speed on CD/DVD burner device
...
This function closes session on on CD/DVD burner device after something was written using 
...
Please see the TrackAtOnceFromTree and TrackAtOnceFromFile samples that will demonstrate how ISO9660 or Joliet file system 
image can be burnon the CD/DVD media and with the help of the StarBurn_CdvdBurnerGrabber_TrackAtOnceFromTree,
...
This example allocates CdvdBurnerGrabber object, records the ISO9660 or Joliet file system image to the CD/DVD media, closes 
...
This function ejects the media on CD/DVD burner device.
...
Please see the TrackAtOnceFromTree and TrackAtOnceFromFile samples that will demonstrate how ISO9660 or Joliet file system 
image can be burn on the CD/DVD media and how StarBurn_CdvdBurnerGrabber_Eject can be used to eject the CD/DVD media to initiate file system 
unmount/mount sequence (to refresh contents of just burn CD/DVD media). 
...
This function loads the media on CD/DVD burner device.
...
image can be burn on the CD/DVD media and how StarBurn_CdvdBurnerGrabber_Load can be used to eject the CD/DVD media to initiate 
file system unmount/mount sequence (to refresh contents of just burn CD/DVD media) and load the same media later w/o user's 
assistance. 
...
This function locks the media inside the CD/DVD device.
...
Please see the TrackAtOnceFromFile and TrackAtOnceFromTree sample that will demonstrate how ISO9660 or Joliet file system 
image can be burn on the CD/DVD media. StarBurn_CdvdBurnerGrabber_Lock can be used to lock the CD/DVD media to prevent 
...
This function releases the media inside the CD/DVD device after the media was locked before.
...
This function erases the rewritable media inserted into CD/DVD burner device.
...
Please see the DiscEraser sample that will demonstrate how to erase the disc.
...
This function gets track information of the asked track number on CD/DVD burner device.
...
p__PTRACK_INFORMATION - Pointer to the track information.
...
CD/DVD media and how StarBurn_CdvdBurnerGrabber_GetTrackInformation can be used to get "invisible" track information on the 
CD/DVD media to check parameters of the track for validness (is track blank, number of free logical blocks etc).
...
CDB_FAILURE_INFORMATION, TRACK_INFORMATION, DISC_INFORMATION
...
StarBurn_CdvdBurnerGrabber_GetTrackInformation(
    ...
    OUT PTRACK_INFORMATION p__PTRACK_INFORMATION
    );


/*

This function gets disc information of the currently inserted disc on CD/DVD burner device.
...
p__PDISC_INFORMATION - Pointer to the disc information.
...
Please see the TrackAtOnceFromTree sample that will demonstrate how ISO9660 or Joliet file system image can be burn on the 
CD/DVD  media and how StarBurn_CdvdBurnerGrabber_GetDiscInformation can be used to get current disc information on the CD/DVD 
...
CDB_FAILURE_INFORMATION, DISC_INFORMATION, TRACK_INFORMATION
...
StarBurn_CdvdBurnerGrabber_GetDiscInformation(
    ...
    OUT PULONG p__PULONG__SystemError,
    OUT PCDB_FAILURE_INFORMATION p__PCDB_FAILURE_INFORMATION,
    OUT PDISC_INFORMATION p__PDISC_INFORMATION
    );


/*

This function gets TOC information of the currently inserted disc on CD/DVD burner device.
...
CD/DVD media and how StarBurn_CdvdBurnerGrabber_GetTOCInformation can be used to get current TOC information on the CD/DVD 
...
This function grabs track from the currently inserted disc on CD/DVD burner device.
...
Please see the GrabTrack sample that will demonstrate how ISO9660 or Joliet file system image can be grabbed from the CD/DVD 
media and how StarBurn_CdvdBurnerGrabber_GrabTrack can be used to grab track actually from the CD/DVD media.
...
This function grabs range of logical blocks from the currently inserted disc on CD/DVD burner device.
...
p__LONG__StartingLBA - Begining of the LBA range to grab.
...
Please see the GrabTrack sample that will demonstrate how ISO9660 or Joliet file system image can be grabbed from the CD/DVD 
media and how StarBurn_CdvdBurnerGrabber_GrabTrack can be used to grab track actually from the CD/DVD media. To grab not whole
...
this StarBurn_CdvdBurnerGrabber_GrabRange can be used to grab range of logical blocks. Attention! Care shuld be taken b/s the
...
This function grabs disc image from the currently inserted disc on CD/DVD burner device and store it in MDS format.
...
p__READ_MODE - Read mode (raw, raw + PQ subchannel or raw + raw P-W subchannel).
...
Please see the GrabDisc sample that will demonstrate how whole disc image can be grabbed from the CD/DVD media and how 
StarBurn_CdvdBurnerGrabber_GrabCD can be used to grab disc from the CD/DVD media.
...
This function grabs disc image from the currently inserted disc on CD/DVD burner device and store it in MDS format, disc image
will be stored into the serie of files.
...
Please see the GrabDisc sample that will demonstrate how whole disc image can be grabbed from the CD/DVD media and how 
StarBurn_CdvdBurnerGrabber_GrabDVD can be used to grab disc from the CD/DVD media. For now only DVD media can be grabbed
...
// Try to grab the disc with 2 retries on bad block hit and single read status report per every 1 second, store all the CD/DVD
// image into serie of files 1024MB (1GB) each
l__EXCEPTION_NUMBER =
...
This function probes supported read modes on the currently inserted disc on CD/DVD burner device.
...
p__BOOLEAN__IsRawPQ - Is raw + PQ subchannel mode supported.

p__BOOLEAN__IsRawRawPW - Is raw + raw P-W subchannel mode supported.

p__BOOLEAN__IsPQ - Is PQ subchannel mode supported.

p__BOOLEAN__IsRawPW - Is raw P-W subchannel mode supported.
...
Please see the burning samples that will demonstrate how disc can be recorded and how StarBurn_CdvdBurnerGrabber_ProbeSupportedWriteModes 
can be used to determine supported write modes.
...
Please see the FindDevice sample that will demonstrate how to find CD/DVD devices installed in the system.


Example

This example finds first first CD/DVD device, allocates CdvdBurnerGrabber object, and destroys the device object after 
it's not needed any more.
...
This function records VCD/MPEG1 image located in a file on the hard disk with current write speed on CD/DVD burner device in
Track-At-Once mode.
...
Please see the VideoCD sample that will demonstrate how VCD/MPEG1 image can be burn on the CD/DVD media and with the help of 
StarBurn_CdvdBurnerGrabber_VideoCD with currently set write speed and currently set optimum power calibration. 


Example

This example allocates CdvdBurnerGrabber object, records the VCD/MPEG1 image to the CD/DVD media and destroys the device object 
after it's not needed any more.
...
Please see the VideoCDEx sample that will demonstrate how VCD/MPEG1 image can be burn on the CD/DVD media and with the help of 
...
This function records SVCD/MPEG2 image located in a file on the hard disk with current write speed on CD/DVD burner device.
...
Please see the SuperVideoCD sample that will demonstrate how SVCD/MPEG2 image can be burn on the CD/DVD media and with the help of 
StarBurn_CdvdBurnerGrabber_SuperVideoCD with currently set write speed and currently set optimum power calibration. 


Example

This example allocates CdvdBurnerGrabber object, records the SVCD/MPEG2 image to the CD/DVD media and destroys the device 
object after it's not needed any more.
...
This function records SVCD/MPEG2 image located in a file on the hard disk with current write speed on CD/DVD burner device in
selected write mode.
...
Please see the SuperVideoCDEx sample that will demonstrate how SVCD/MPEG2 image can be burn on the CD/DVD media and with the help of 
StarBurn_CdvdBurnerGrabber_SuperVideoCDEx with currently set write speed and currently set optimum power calibration. 


Example

This example allocates CdvdBurnerGrabber object, records the SVCD/MPEG2 image to the CD/DVD media and destroys the device object 
after it's not needed any more.
...
This function returns inserted to CD/DVD burner device disc type. This data can be used to report it to the user and to select
...
Please see the TrackAtOnceFromTree and TrackAtOnceFromFile samples that will demonstrate how ISO9660 or Joliet file system 
image can be burn on the CD/DVD media and what use can be taked from inserted disc type information.


Example

This example allocates CdvdBurnerGrabber object, retreives inserted disc type and destroys the device object after it's not 
needed any more.
...
This function cancels current read or write process on the CD/DVD burner. 
...
result burn to the CD/DVD media.
...
whole result burn to the CD/DVD media.
...
This function records ISO9660 or Joliet file system image located in file tree object with current write speed on CD/DVD burner device in Disc-At-Once PQ mode.
...
Please see the DiscAtOnceFromTree sample that will demonstrate how ISO9660 or Joliet file system image located in a file tree object can be burn on the CD/DVD 
...
This example allocates CdvdBurnerGrabber object, creates ISO9660 or Joliet file system image located in a file tree objeect and burns it to the CD/DVD media and 
destroys the device object after it's not needed any more.
...
This function records ISO9660 or Joliet file system image located in file tree object with current write speed on CD/DVD burner device in Disc-At-Once 
raw P-W mode.
...
StarBurn_CdvdBurnerGrabber_DiscAtOnceRawPWFromFile, StarBurn_Destroy, StarBurn_CdvdBurnerGrabber_Create, 
...
//
// Generic undecorated code (begin)
//


//
// StarBurn_DVDVideo_Create() function is not documented
//
...
//
// StarBurn_DVDVideo_Destroy() function is not documented
//
...
//
// StarBurn_DVDVideo_SeekToBegin() function is not documented
//
...
//
// StarBurn_DVDVideo_Read() function is not documented
//
...
//
// StarBurn_DVDVideo_GetSizeInUCHARs() function is not documented
//
...
//
// StarBurn_DVDVideo_GetTreePointer() function is not documented
//
...
//
// StarBurn_ISO9660JolietFileTree_GetFileSizeInUCHARs() function is not documented
//
...
//
// StarBurn_ISO9660JolietFileTree_GetNodeSystemTime() function is not documented
//
...
//
// StarBurn_ISO9660JolietFileTree_GetNodeISO9660DateTime() function is not documented
//
...
//
// StarBurn_IsAudioFileSupported() function is not documented
//
...
//
// StarBurn_GetAudioFileStreamSizeInUCHARs() function is not documented
//
...
//
// StarBurn_ISO9660JolietFileTree_GetFullPath() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_ReadATIP() function is not documented
// 
...
//
// StarBurn_CdvdBurnerGrabber_MSFToLBA() function is not documented
// 
...
//
// StarBurn_GetDVDPLUSRDLCompatibleMode() function is not documented
//
...
//
// StarBurn_SetDVDPLUSRDLCompatibleMode() function is not documented
//
...
//
// StarBurn_GetDVDPadding() function is not documented
//
...
//
// StarBurn_SetDVDPadding() function is not documented
//
...
//
// StarBurn_GetEjectAfterFail() function is not documented
//
...
//
// StarBurn_SetEjectAfterFail() function is not documented
//
...
//
// StarBurn_GetFastReadTOC() function is not documented
//
...
//
// StarBurn_SetFastReadTOC() function is not documented
//
...
//
// StarBurn_GetBufferUnderrunTimeOutInMs() function is not documented
//
...
//
// StarBurn_SetBufferUnderrunTimeOutInMs() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_DiscBasicInformation_Create() function is not documented
//
...
StarBurn_CdvdBurnerGrabber_DiscBasicInformation_Create(
    ...
    OUT PULONG p__PULONG__SystemError,
    OUT PCDB_FAILURE_INFORMATION p__PCDB_FAILURE_INFORMATION,
	OUT PVOID *p__PPVOID__DiscBasicInformation
	);


//
// StarBurn_CdvdBurnerGrabber_DiscBasicInformation_Destroy() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_GetDVDRegionMask() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_GetRPC() function is not documented
//
...
//
// StarBurn_ISO9660JolietFileTree_AddEx() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_VerifyTree() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_VerifyTreeEx() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_VerifyFile() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_StopPlayScan() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_TrackAtOnceFromPipeEx() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_TrackAtOnceFromMemory() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_CreateEx() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_VideoCDExEx() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_SuperVideoCDExEx() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_SessionAtOnceRawRawPWEx() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_GetLastTrack() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_ExecuteGeneric() function is not documented
//
...
//
// StarBurn_ISO9660JolietFileTree_AddW() function is not documented
//
...
StarBurn_ISO9660JolietFileTree_AddW(
    ...
    IN PCHAR p__PCHAR__RootDirectoryOrFileNewName,
    IN FILE_TIME p__FILE_TIME,
    IN PVOID *p__PPVOID__ISO9660JolietFileTreeNode__Parent,
	OUT PVOID *p__PPVOID__ISO9660JolietFileTreeNode__NewChild
    );
...
//
// StarBurn_CdvdBurnerGrabber_TestUnitReadyExEx() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_GetMediaTrayStatus() function is not documented
//
...
//
// StarBurn_CdvdBurnerGrabber_SetReadSpeed() function is not documented
//
...
//
// StarBurn_GetDeviceTimeOutByDeviceAddress() function is not documented
//
...
//
// StarBurn_SetDeviceTimeOutByDeviceAddress() function is not documented
//
...
//
// StarBurn_GetId() function is not documented
//
